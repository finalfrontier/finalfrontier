...
function ENT:InitPostEntity()
	...
	if SERVER then
		...
		self:SetNWTable("corners", self._corners)
	...
	if SERVER then
		...
		self:SetNWFloat("shields", math.random())
		self._lastupdate = CurTime()

		self._players = {}
	end
	
	if self:GetSystemName() then
...
if SERVER then
	...
	function ENT:Think()
		...
		self:SetNWFloat("temperature", self:GetTemperature() *
			(1 - TEMPERATURE_LOSS_RATE * self:GetSurfaceArea() * dt))

		local min = Vector(self._bounds.l, self._bounds.t, -65536)
		local max = Vector(self._bounds.r, self._bounds.b, 65536)
	...
	function ENT:AddCorner(index, x, y)
		self._corners[index] = { x = x, y = y }
		self._bounds:AddPoint(x, y)
		self.Ship.GetBounds():AddPoint(x, y)
...
function ENT:GetAirVolume()
	return self:GetNWFloat("airvolume")
end
...
function ENT:GetShields()
	return self:GetNWFloat("shields")
end
